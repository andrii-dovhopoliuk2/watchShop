...
class CategoryController extends AppController {
    ...
    public function deleteAction(){
        ...
        $errors = '';
        if($children){
            $errors .= '<li>Видалення неможливе, в категорії є вложені категорії</li>';
        }
    ...
    public function editAction(){
        if (!empty($_POST)) {
            ...
            if ($category->update('category',$id)) {
                ...
                \R::store($category);
                $_SESSION['success'] = 'Зміни успішно застосовані';
            }
            redirect();
        ...
        $id = $this->getRequestID();
        $category = \R::load('category',$id);
        App::$app->setPropeties('parent_id',$category->parent_id);
