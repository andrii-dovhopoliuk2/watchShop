$('.delete').click(function () {
    var res = confirm("Ви впевнені, що хочете це зробити?");
    if (!res) {
        return false;
    }
});
...
$('.deleteForCache').click(function(){
    var res = confirm("Ви точно хочете очистити кеш?");
    if (!res){
        return false;
    }
});
...
$(".select2").select2({
    placeholder: "Начніть вводити назву товару",
...
if(buttonSingle){
    ...
    onSubmit: function(file, ext){
        if (! (ext && /^(jpg|png|jpeg|gif)$/i.test(ext))){
            alert('Помилка! Дозволені тільки картинки.');
            return false;
        }
...
if (buttonMulti){new AjaxUpload(buttonMulti, {
